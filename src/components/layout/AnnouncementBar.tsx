      ...
      <AnimatePresence mode="wait">
        {isExpanded ? (
          <motion.div
            ...
            className="fixed top-6 z-[45] h-10 flex items-center rounded-full px-4 shadow-[0_0_20px_rgba(0,0,0,0.8)] max-w-[calc(100vw-3rem)] overflow-hidden"
            style={{
              left: "50%",
          ...
          <motion.button
            ...
            animate={
              isAlarming 
                ? { y: 0, opacity: 1, x: "-50%", rotate: [-8, 8, -8, 8, 0], scale: 1.1 } 
                : { y: 0, opacity: 1, x: "-50%", rotate: 0, scale: 1 }
            }
            exit={{ y: -52, opacity: 0, x: "-50%" }}
            transition={{ duration: 0.4 }}
            onClick={() => setIsExpanded(true)}
            className="fixed top-6 z-[45] w-10 h-10 rounded-full flex items-center justify-center cursor-pointer shadow-[0_0_20px_rgba(212,175,55,0.2)] border-[1.5px] border-gold backdrop-blur-md bg-black/30 group hover:bg-black/60 transition-colors"
            style={{ left: "50%" }}
